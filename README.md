...
Для совмести поле teacher не удалено. Удалена только связь один ко многим.
...
Для переноса информации об учетелях в новую модель
```bash
python manage.py restruct_students
```